##---------------------------------------------------------------------#
##
## Title:       ANNUAL VARIATION IN EARLY-LIFE SURVIVAL
##
## Purpose of script: Describing variance in early-life survival among 
## Wytham Woods' nestbox-breeding populations of great tits and blue
## tits. 
...
## Author: Simon Evans
##
## Date Created: 2023-01-06 (building atop a script written to generate
## analyses presented at the 2022 Hole-Nesting Birds conference,
## titled, 'Opportunity for selection analyses.R')
##
...
bt.data <- read.csv( "~/Library/CloudStorage/OneDrive-UniversityofExeter/Research/Prenatal survival/Ecology Letters special issue/bt data (22Sept23).csv")
bt.pulli <- read.csv()
...
## Ringing data ----
#ringing.data <- read.csv("~/Library/CloudStorage/OneDrive-UniversityofExeter/Research/Prenatal survival/Ecology Letters special issue/GretiBlutiHistorical.csv", header = T); dim(ringing.data)  # No nest locations in this file
...
# Do we have a unique identifier for each datum?
stopifnot(length(unique(ringing.data$X)) == dim(ringing.data)[1])
...
### Blue tits ----
bt.pulli <- ringing.data[which(ringing.data$bto_species_code == "BLUTI" & ringing.data$age == "1" & ringing.data$year >= 1960),]; dim(bt.pulli)
length(unique(bt.pulli$bto_ring))
bt.appearance.count <- rev(sort(table(bt.pulli$bto_ring))); head(bt.appearance.count)

#### Dealing with replicates ----
## Two ringnumbers appear more than once in the blue tit data: T641298 (linked to pnums = 20051B68 & 20051MP64) and F491535 (linked to pnums = 19891C34 & 19891C9A)
##
## F491535: ringnumber fits with other pulli ringed at 19891C34. For 19891C9A it is the only entry and therefore I delete this.
bt.pulli <- bt.pulli[bt.pulli$X != bt.pulli[bt.pulli$pnum == "19891C9A",]$X,]; dim(bt.pulli)

## T641298: hard to correct because it is the last in the ringing sequence at one nest (20051MP64) and the first at another (20051B68)
## Is it recorded as a breeder?
dim(bt.data[which(bt.data$Father == "T641298" | bt.data$Mother == "T641298" | bt.data$Identified.adults == "T641298"),])[1]
# NO. So can ignore the replication as it won't wrongly contribute to successful recruitment from either nest.
...
### Eggs that did not generate ringed chicks ----
...
## Blue tits ----
bt.data <- all.data[which(all.data$Species == "b" & !is.na(all.data$year)),]; length(bt.data$Pnum)

# Data checking
table(bt.data$Num.eggs.manipulated)
bt.data <- bt.data[bt.data$Num.eggs.manipulated == 0,]; table(bt.data$Num.eggs.manipulated)
table(bt.data$Clutch.size)
table(bt.data$Num.fledglings)
bt.data[bt.data$Num.fledglings < 0,]; bt.data[which(bt.data$Num.fledglings < 0 & is.na(bt.data$Mean.chick.weight)),]$Num.fledglings <- 0; bt.data <- bt.data[bt.data$Num.fledglings >= 0,]; table(bt.data$Num.fledglings)
table(bt.data$Clutch.size - bt.data$Num.fledglings)
bt.data <- bt.data[bt.data$Clutch.size - bt.data$Num.fledglings >= 0,]; table(bt.data$Clutch.size - bt.data$Num.fledglings)

bt.egg.counts <- aggregate(bt.data$Clutch.size, by = list(bt.data$year), FUN = function(x) {sum(x[!is.na(x)])}); colnames(bt.egg.counts)<- c("year", "egg.total")
bt.fledgling.counts <- aggregate(bt.data$Num.fledglings, by = list(bt.data$year), FUN = function(x) {sum(x[!is.na(x)])}); colnames(bt.fledgling.counts) <- c("year", "fledgling.total")
bt.nest.count <- aggregate(bt.data$year, by = list(bt.data$year), FUN = function(x) {length(x[!is.na(x)])}); colnames(bt.nest.count) <- c("year", "nest.count")
bt.annual.data <- cbind(bt.nest.count, bt.egg.counts$egg.total, bt.fledgling.counts$fledgling.total, bt.fledgling.counts$fledgling.total/bt.egg.counts$egg.total, (1-(bt.fledgling.counts$fledgling.total/bt.egg.counts$egg.total))/(bt.fledgling.counts$fledgling.total/bt.egg.counts$egg.total)); colnames(bt.annual.data) <- c("year", "nest.count", "egg.total", "fledgling.total", "w1", "I1")

dev.off(); 
plot(w1 ~ year, data = bt.annual.data)
lines(smooth.spline(x = bt.annual.data$year, y = bt.annual.data$w1, df = 10))
plot(I1 ~ year, data = bt.annual.data)
lines(smooth.spline(x = bt.annual.data$year, y = bt.annual.data$I1, df = 10))
...
### Males ----
recruited.male.bt.fledglings <- aggregate(!is.na(match(bt.pulli$bto_ring, bt.data$Father)), by = list(bt.pulli$year), FUN = sum)
colnames(recruited.male.bt.fledglings) <- c("year", "total"); recruited.male.bt.fledglings

### Females ----
recruited.female.bt.fledglings <- aggregate(!is.na(match(bt.pulli$bto_ring, bt.data$Mother)), by = list(bt.pulli$year), FUN = sum)
colnames(recruited.female.bt.fledglings) <- c("year", "total"); recruited.female.bt.fledglings

#### Sexes combined ----
recruited.bt.fledglings <- data.frame(recruited.male.bt.fledglings$year, recruited.male.bt.fledglings$total + recruited.female.bt.fledglings$total, recruited.female.bt.fledglings$total/recruited.male.bt.fledglings$total)
colnames(recruited.bt.fledglings) <- c("year", "count", "sex ratio (f:m)")  # Sex ratio is used to highlight where, in particular, catching effort for male breeders did not match females.
recruited.bt.fledglings
## For blue tits, data identifying breeders must be very poor because in some years (e.g., through 1990s) none of the pulli are ever
## recorded as breeders. This is, then, a major limitation of Wytham Woods data for analyses of blue tit post-fledging survival.
## Sex-ratios of identified recruits are also highly skewed until 2001, such that only for 2001-2011 can I be confident that recruitment
## data are reliable.
